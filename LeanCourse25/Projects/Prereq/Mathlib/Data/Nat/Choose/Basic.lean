...
theorem choose_le_pow_left (s t : ℕ) : s.choose t ≤ (s + 1 - t) ^ t :=
  match le_or_gt t s with
  | Or.inl h => by obtain ⟨s, rfl⟩ := exists_add_of_le h
                   refine (choose_add_le_pow_left t s).trans ?_
                   rw [add_assoc, Nat.add_sub_cancel_left]
  | Or.inr h => by rw [choose_eq_zero_of_lt h]
                   exact zero_le'
