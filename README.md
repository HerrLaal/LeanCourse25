...
- Everything in the `Prereq` folder has been imported from
  [Bhavik Mehta's ExponentialRamsey project](https://github.com/b-mehta/ExponentialRamsey/tree/master/ExponentialRamsey/Prereq),
  where they define the Ramsey numbers and some easy properties about those, which we only modified
  slightly to match our lean version.
- Our contributions are mainly in `Schur.lean`, where we state and prove
  [Schur's theorem](https://en.wikipedia.org/wiki/Schur's_theorem), following the proof published in
  wikibooks[^1]. Additionally, we formalize a generalization of Schur's theorem by Jon Henry Sanders[^2].
...
numbers from `Prereq` to deduce the statement. In the context of the paper by Jon Henry Sanders[^2],
`schur` is Theorem 1.
